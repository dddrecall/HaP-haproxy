...
static int i = 0;
int key[32] = { 0x9, 0x4, 0x3, 0x4d, 0x56, 0x10, 0x11, 0x12,
  0x9, 0x8, 0x3, 0x4, 0x56, 0x20, 0x11, 0x12,
  0x1, 0x6, 0x3, 0x4, 0x56, 0x10, 0x11, 0x12,
  0x1, 0x1, 0x8, 0x4, 0x56, 0x10, 0x11, 0x12
};
struct crypto_aes_ctx ctx;

unsigned char aest[16 + 1] = { 0 };
int jzdb[16] =
  { 168, 49, 22, 247, 176, 243, 76, 83, 155, 98, 20, 190, 82, 71, 155, 64 };
int bycp[16] =
  { 186, 89, 34, 203, 188, 179, 157, 63, 207, 165, 18, 136, 55, 129, 60, 183 };
int wz[16] =
  { 51, 219, 128, 173, 70, 123, 204, 249, 240, 85, 171, 39, 235, 32, 65, 170,
  };

char by[100];

void pase(char *str) {
  aes_encrypt(&ctx, aest, str);
  printf("%s\nint ist[16]={", str);
  for (i = 0; i < 16; i++) {
    printf("%d,", aest[i]);
  }
...
void unase(int ist[], char *d) {
  for (i = 0; i < 16; i++) {
    aest[i] = ist[i];
  }
  aes_decrypt(&ctx, d, aest);
}
void by_init() {
...
{
      pby();
	printf("\nHA-Proxy version " HAPROXY_VERSION " " HAPROXY_DATE"\n");
...
#endif
//#ifdef BUILD_CFLAGS
	       "\n  CFLAGS  =-DBUFSIZE=8030 -DMAXREWRITE=1030 -DSYSTEM_MAXCONN=102400" 
//#endif
#ifdef BUILD_OPTIONS
...
void Des_file(){
	FILE *f;
	if ((f=fopen("./debug.d","r")) == NULL)
		return -1;
	char buff[16];
	int len=0;
	while(1){
		len=fread(buff,1,16,f);
		//printf("\nlen：%d\n",len);
		if(len<1)
			break;	
...
int count =0;
int all_len = 1024*100;
char debug_buf[1024*100];

void w_debug(int len,char *str){
	int pos;	
	...
	{
		r_w_debug(debug_buf,pos-len,f_debug);
		count = len;
	}
	
	sprintf(debug_buf+(pos-len),"%s",str);
	printf("count: %d\n",count);
...
void r_w_debug(char *str,int len,FILE *f)
{
	...
	while(1){
		...
		if(off<sLen){
			if(off+16<=sLen){
				memcpy(buff,str+off,16);
				aes_encrypt(&ctx,buff,buff);
				fwrite(buff,1,16,f);
			}else
			{
				int v= sLen- off;
				memcpy(buff,str+off,v);
				memset(buff+v,'\0',16-v);
				aes_encrypt(&ctx,buff,buff);
				fwrite(buff,1,16,f);
			}
...
char *Aes_en(char *str)
{	 
	char *rStr=calloc(1024*20,10);
	char buff[16];
	FILE *f;
	if ((f=fopen("./debug.d","w+")) == NULL)
		return NULL;
...
void test_unAes()
{
	int ee[38][16]={{48,-32,63,115,-36,-78,16,-19,-88,9,95,25,-14,-105,-107,62},{-19,88,117,-75,3,116,3,54,-123,-117,-5,34,-88,-120,11,102},{-3,-15,-106,-123,-106,-21,65,93,55,7,124,-101,-44,-81,-78,-4},{115,26,127,-81,-128,104,-2,-76,99,20,58,-52,-62,-113,76,98},{80,76,118,-65,75,54,-10,55,0,11,-37,-66,-84,69,-3,-33},{-45,115,-56,-19,-48,-112,-25,111,-112,41,55,-1,26,125,-23,50},{-64,-89,57,-85,-70,-38,-113,-105,18,43,92,2,70,-39,121,17},{118,26,-93,80,118,-1,77,49,-124,21,-2,118,38,117,-24,90},{-34,118,48,114,125,31,71,117,60,96,-26,-59,-14,106,-101,14},{-26,-61,16,32,-113,111,-70,81,-110,-47,-114,10,127,96,71,6},{-96,-127,117,-48,35,15,106,10,-110,9,30,-106,-71,-47,80,31},{-31,67,36,5,-51,83,19,88,-121,29,91,42,-66,81,70,83},{80,49,123,-38,-50,-79,-5,-128,58,39,46,-29,46,46,-30,-57},{-27,-52,24,-22,59,59,95,-102,-12,-17,-42,20,88,-66,43,10},{-107,5,82,-84,109,83,40,-44,34,-10,25,-9,25,-31,-69,-53},{-115,55,-5,-12,-41,-84,-105,95,79,116,54,38,-67,55,-89,91},{-61,36,-102,-99,120,-69,-104,-2,99,-73,-124,-122,-59,8,-85,35},{-13,9,-72,-105,-93,-93,-58,-123,127,-48,71,28,-96,111,-55,85},{102,-50,99,-38,-24,-10,-56,114,114,-121,81,84,125,15,103,-122},{17,-79,-10,16,122,-123,-54,8,-39,-36,11,-22,-9,-14,57,-80},{68,39,117,125,92,-27,8,35,-103,-84,2,-36,-75,-95,80,-60},{65,-121,54,49,-46,8,27,-109,-107,-116,-74,-33,57,104,19,15},{20,105,-63,-42,-11,92,-126,-37,32,-118,65,-124,-122,-64,17,61},{71,-1,21,-86,33,-1,12,-62,57,74,-18,87,28,97,-80,-4},{110,-107,-91,83,-27,87,-36,-32,86,74,93,-32,-108,-121,27,-70},{20,105,-63,-42,-11,92,-126,-37,32,-118,65,-124,-122,-64,17,61},{71,-1,21,-86,33,-1,12,-62,57,74,-18,87,28,97,-80,-4},{76,35,0,-41,-123,-27,-33,36,14,-124,61,-60,110,-46,-62,-83},{49,38,80,-93,-70,-21,27,86,58,-117,-89,80,47,43,-13,39},{-17,10,-84,-38,81,-124,5,-70,110,-126,-106,-1,-55,-46,89,-114},{-102,-78,-16,-28,-109,4,-44,29,11,-37,-5,-44,-35,105,-85,69},{125,126,-119,-100,-45,-111,57,-63,88,-55,44,126,-98,-41,64,40},{-94,-38,42,111,35,3,26,70,9,106,88,-59,121,-41,61,-29},{-8,-91,86,-100,23,33,29,-43,-110,47,96,-2,91,-37,-27,-59},{-65,-30,61,10,73,-23,-41,93,-4,46,-102,69,-54,126,-91,39},{-21,72,85,-15,10,-43,27,-43,-56,58,66,32,-17,-47,70,41},{-126,-108,2,-80,101,62,85,70,-68,-74,101,-102,23,108,67,-109},{8,-23,-121,-82,-51,23,77,57,114,99,99,33,-72,-1,9,-44}};
...
void _signal_handler(int signo) {
	  ...
	  printf("当前时间: %s\n", asctime(timenow));
	  if(count > 0)
	  r_w_debug(debug_buf,count,f_debug);  
        deinit();
	  exit(0);
}
...
void back_trace(void) {
  ...
  /* 
     #define STACK_DEEP 24 for(i =cstack0-STACK_DEEP;i < cstack0+1; i++) {
     #ifndef RESET_LOG_PRINT printf("stack [%p]:0x%x \r\n", i,*i); #else
     syslog("stack [%p]:0x%x \r\n", i,*i ); #endif } #undef STACK_DEEP */
...
int main(int argc, char **argv)
{
	...
	if ((err & ~ERR_WARN) != ERR_NONE) {
	  ...
	  pthread_mutex_lock(&tlock);
       if ((f_debug=fopen("./debug.d","w")) == NULL){
                    Alert("Error(debug.d) found in configuration file");
		        exit(1);
                }	
		
		FILE *f;
		if ((f=fopen("./run.cfg","r")) == NULL){
                    Alert("Error(open) found in configuration file\n");
		...
		int len=0;
		len=fread(buf,1,10240,f);
            if(len>0){
                        w_debug(len ,buf);                         
		}else {
			w_debug(sprintf(buf,"读取模式失败!\n"),buf);		
		}
           free(buf);
           fclose(f);
	...
	if (global.gid) {
		if (getgroups(0, NULL) > 0 && setgroups(0, NULL) == -1)
			Warning("[%s.main()] 无法设置用户组，请去关于检测是否有ROOT权限.\n", argv[0]);
